...
export class AgentOrchestrator {
  ...
  private async executeStep(
    ...
    try {
      ...
      const invocationContext: InvocationContext = {
        domain: workflow.context?.domain || 'default',
        workspace: workflow.context?.workspace,
        task: {
          type: 'workflow-step',
          workflowId: workflow.id,
          stepId: step.id,
      ...
      return {
        ...
        result,
        state: await this.stateStore.get(step.agentId),
  ...
  async spawnAgent(
    ...
    await this.stateStore.update(agentId, {
      currentTask: null, // Clear after completion
