...
import type { WorkRequest, Task } from '../domain/types';
...
export class InputValidator {
   ...
   */
  async validateWorkRequest(request: unknown): Promise<ValidationResult<WorkRequest>> {
    try {
      const sanitized = await this.workRequestSchema.parseAsync(request);
      return {
        valid: true,
        sanitized: sanitized as WorkRequest,
      };
    } catch (error) {
   ...
   * @throws ValidationError if validation fails
   */
  async validateTask(task: unknown): Promise<ValidationResult<Task>> {
    try {
      const sanitized = await this.taskSchema.parseAsync(task);
      return {
        valid: true,
        sanitized: sanitized as Task,
      };
    } catch (error) {
      if (error instanceof z.ZodError) {
        const fields = error.errors.map(e => ({
