...
export class WorkflowTracker {
  ...
  constructor(store: WorkflowExecutionStore, config = {}) {
    this.store = store;
    this.config = {
...
class MessageObserver {
  ...
  private subscribeToEvents(): void {
    ...
    this.subscriptions.push(
      this.messageBus.on('published', onPublished),
      this.messageBus.on('delivered', onDelivered),
      this.messageBus.on('failed', onFailed)
    );
  }
