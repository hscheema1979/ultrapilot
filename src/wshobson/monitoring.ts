  ...
  private async detectAnomalies(): Promise<void> {
    ...
    if (errorRate > this.config.alerts.errorRateThreshold) {
      this.addAnomaly({
        ...
        message: `Error rate ${(errorRate * 100).toFixed(1)}% exceeds threshold ${(this.config.alerts.errorRateThreshold * 100).toFixed(1)}%`,
    ...
    if (avgLatency > this.config.alerts.latencyThreshold) {
      this.addAnomaly({
        ...
        severity: avgLatency > this.config.alerts.latencyThreshold * 2 ? 'critical' : 'medium',
        message: `Average latency ${avgLatency.toFixed(0)}ms exceeds threshold ${this.config.alerts.latencyThreshold}ms`,
    ...
    if (successRate < this.config.alerts.minSuccessRate) {
      this.addAnomaly({
        type: 'success_drop',
        severity: successRate < 0.5 ? 'critical' : 'high',
        message: `Success rate ${(successRate * 100).toFixed(1)}% below minimum ${(this.config.alerts.minSuccessRate * 100).toFixed(1)}%`,
        currentValue: successRate,
        threshold: this.config.alerts.minSuccessRate,
        timestamp: Date.now(),
        context: { windowSize: window.length },
      });
    }
